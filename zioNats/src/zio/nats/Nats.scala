...
import io.nats.client.{AuthHandler, Options, Message as JMessage, Nats as JNats}
import zio.*
...
object Nats:
  ...
  private def acquire(config: NatsConfig, additional: Option[Options.Builder => Options.Builder]): ZIO[Scope, Throwable, Nats] =
    ZIO
      .acquireRelease(
